...
using WinFormium.Sources.Formium.Forms.@base.Forms.WindowBase;
...
internal class OffscreenImeHandler
{
    internal static class ImeNative
    {
        ...
        [DllImport("Imm32.dll")]
        internal static extern nint ImmAssociateContextEx(nint hWnd, nint hIMC, uint flag);

        [DllImport("Imm32.dll")]
        public static extern bool ImmSetOpenStatus(nint himc, bool b);
        [DllImport("Imm32.dll")]
        internal static extern bool ImmDestroyContext(nint hIMC);
        ...
        internal static extern uint ImmGetCompositionString(nint hIMC, uint dwIndex, byte[] lpBuf, uint dwBufLen);

        [DllImport("Imm32.dll")]
        internal static extern int ImmSetCompositionWindow(nint hIMC, ref COMPOSITIONFORM lpCompForm);

        [DllImport("Imm32.dll")]
        public static extern int ImmSetCandidateWindow(nint hIMC, [MarshalAs(UnmanagedType.Struct)] ref CANDIDATEFORM lpCandidateForm);
    }
    ...
    internal nint Handle => hWnd.DangerousGetHandle();

    internal WinFormium.Formium Owner { get; }

    public OffscreenImeHandler(WinFormium.Formium owner)
    {
        Owner = owner;
    }
    ...
    internal void GetCompositionSelectionRange(nint imc, out int targetStart, out int targetEnd)
    {
    ...
    internal IEnumerable<CefCompositionUnderline> GetCompositionUnderlines(nint imc, int start, int end)
    {
    ...
    internal List<CefCompositionUnderline> GetCompositionInfo(nint imc, uint lparam, string compositionText, out int compositionStart)
    {
    ...
    internal bool GetString(nint imc, uint lparam, uint type, out string result)
    {
    ...
    {
        CleanupComposition();
        ImeNative.ImmAssociateContextEx(Handle, nint.Zero, 0);
    }
    ...
    public void EnableIME()
    {
        ...
        ImeNative.ImmAssociateContextEx(Handle, nint.Zero, ImeNative.IACE_DEFAULT);
    }
    ...
    public void OnImeComposition(WindowMessage message, nint wParam, nint lParam)
    {
