...
public abstract class JavaScriptWindowBindingObject : CefV8Handler
{
    ...
    #region Local side
    // sync function
    internal protected void RegisterSynchronousNativeFunction(Func<WinFormium.Formium, JavaScriptArray, JavaScriptValue?> functionDelegate)
    {
        RegisterSynchronousNativeFunction(functionDelegate.Method.Name, functionDelegate);
    }

    internal protected void RegisterSynchronousNativeFunction(string functionName, Func<WinFormium.Formium, JavaScriptArray, JavaScriptValue?> functionDelegate)
    {
        WindowBindingFunctions.Add(new JavaScriptWindowBindingObjectFunction(functionDelegate) { FunctionName = functionName });
    }

    // async function
    internal protected void RegisterAsynchronousNativeFunction(Action<WinFormium.Formium, JavaScriptArray, JavaScriptPromise> functionDelegate)
    {
        RegisterAsynchronousNativeFunction(functionDelegate.Method.Name, functionDelegate);
    }
    internal protected void RegisterAsynchronousNativeFunction(string functionName, Action<WinFormium.Formium, JavaScriptArray, JavaScriptPromise> functionDelegate)
    {
        WindowBindingFunctions.Add(new JavaScriptWindowBindingObjectFunction(functionDelegate) { FunctionName = functionName });
    }
