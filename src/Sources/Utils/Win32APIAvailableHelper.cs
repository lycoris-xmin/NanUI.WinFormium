...
internal class Win32APIAvailableHelper
{
    ...
    private static SafeHINSTANCE? LoadLibraryFromSystemPathIfAvailable(string libraryName)
    {
        ...
        if (!kernel32.IsNull)
        {
            if (GetProcAddress(kernel32, "AddDllDirectory") != nint.Zero)
            {
                module = LoadLibraryEx(libraryName, nint.Zero, LoadLibraryExFlags.LOAD_LIBRARY_SEARCH_SYSTEM32);
            }
            else
            {
